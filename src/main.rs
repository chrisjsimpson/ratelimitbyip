                  ...
                  Ok(client_ip) => {
                    ...
                    let mut process_client_ip = || {
                      ...
                      if !clients.contains_key(&client_ip) {
                        ...
                        let ratelimiter = Ratelimiter::builder(10, Duration::from_secs(5))
                        .max_tokens(10)
                        .initial_available(5)
                        //.initial_available(5)
                        .build()
                      ...
                      match clients.get(&client_ip) {
                        Some(client_rate_limiter ) => { 
                          ...
                          match client_rate_limiter.try_wait() {
                              Ok(_) => { 
                                  eprintln!("Proceed! {} Not rate limited yet!", client_ip);
                                  const APACHE_REWRITEMOD_DOCS: &str = "https://httpd.apache.org/docs/2.4/rewrite/rewritemap.html#prg";
                                  eprintln!("four-character string 'NULL' (to imply this client_ip/request is not rate limited)");
                                  eprintln!("See docs {}", APACHE_REWRITEMOD_DOCS);
                                  println!("NULL");
                              },
                              Err(duration  )  => {
